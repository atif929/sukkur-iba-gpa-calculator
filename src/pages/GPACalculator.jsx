...
export default function GPACalculator() {
  ...
  const updateCourse  = (i, u) => setCourses(p => p.map((c, idx) => idx === i ? u : c));
  const addCourse     = () => setCourses(p => [...p, defaultCourse()]);
  const removeCourse  = (i) => { if (courses.length > 1) setCourses(p => p.filter((_, idx) => idx !== i)); };
      ...
      <div ref={pageRef} className="container section">
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <button className="btn btn-ghost btn-sm"
            onClick={() => { setCourses([defaultCourse(), defaultCourse(), defaultCourse()]); setResult(null); setShowErrors(false); }}>
            🗑 Clear All
          </button>
        </div>
