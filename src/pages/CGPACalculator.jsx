...
export default function CGPACalculator() {
  ...
  const updateSemester = (i, field, value) =>
    setSemesters(p => p.map((s, idx) => idx === i ? { ...s, [field]: value } : s));
  const addSemester    = () => setSemesters(p => [...p, defaultSemester(p.length + 1)]);
  const removeSemester = (i) => { if (semesters.length > 1) setSemesters(p => p.filter((_, idx) => idx !== i)); };

  const validate = () => {
    const errs = {};
    semesters.forEach((s, i) => {
      const gpa = parseFloat(s.gpa), ch = parseFloat(s.creditHours);
  ...
  return (
    <PageWrapper>
      <div ref={pageRef} className="container section">
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
          ...
          <button className="btn btn-ghost btn-sm"
            onClick={() => { setSemesters([defaultSemester(1), defaultSemester(2)]); setResult(null); setShowErrors(false); }}>
            🗑 Clear All
          </button>
        </div>
